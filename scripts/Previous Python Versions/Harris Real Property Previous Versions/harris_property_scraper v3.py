# --- Full script with Limits Adjusted for Wider Scrape (v9) ---
import asyncio
...
# --- MODIFIED Constants for wider scrape ---
MAX_PAGES_PER_NAME_SEARCH = 25          # Max pages to paginate for a single name search (Increased from 10)
...
# MAX_MAIN_RECORDS_TO_EXTRACT_PER_LEAD_TEST_MODE is no longer used to break loops
...
def parse_plain_text_legal_description(text_content: str, record_file_number_for_log: str, page_num_for_log: int, main_row_k_for_log: int) -> dict:
    ...
    if not any(val for key, val in legal_data.items() if key != "legal_description_text" and val) and text_content and not legal_data.get("legal_description_text"):
    ...
    for key_rd_clean in legal_data: 
        if isinstance(legal_data[key_rd_clean], str) and "Related Docs" in legal_data[key_rd_clean]:
            ...
            if original_value != cleaned_value: ts_print(f"    {log_prefix} Cleaning 'Related Docs' from {key_rd_clean}: '{original_value}' -> '{cleaned_value}'"); legal_data[key_rd_clean] = cleaned_value
    return legal_data
...
def extract_data_from_current_page_rp(table_locator: Locator, page_num: int) -> list:
    ...
    while k < num_total_trs:
        ...
        if is_main_record_row:
            ...
            ts_print(f"  [MAIN_RECORD_DEBUG P{page_num}R{k+1}] Actual File# Idx: {idx_file_number_actual}, Date Idx: {idx_file_date_actual}, Type Idx: {idx_type_vol_page_actual}, Expected Legal Idx (HTML): {idx_expected_html_legal_col}")
            ...
            try:
                    ...
                    try:
                        ...
                        if "<table" in html_lower and \
                           (any(id_marker.lower() in html_lower for id_marker in ['lblDesc', 'lblBlock', 'lblLot', 'lvLegal', 'lblSubDivAdd']) or \
                            ...
                            ts_print(f"      [LEGAL_ATTEMPT_1 P{page_num}R{k+1}] td[{idx_expected_html_legal_col}] HAS HTML TABLE. Parsing with BS4.")
                            parsed_legal_data = extract_legal_description_from_html_table(html_content_type_A, file_number_text, page_num, k)
                        else: ts_print(f"      [LEGAL_ATTEMPT_1 P{page_num}R{k+1}] td[{idx_expected_html_legal_col}] not structured legal table. Text: '{clean_cell_text(potential_legal_table_cell_loc.inner_text(timeout=500))[:100]}'")
                ...
                if not key_fields_found_A:
                    ...
                    scan_end_idx = min(idx_file_number_actual + 22, num_tds) # MODIFIED SCAN END INDEX
...
def search_rp_for_decedent_and_extract(page: Page, decedent_last: str, decedent_first: str | None, probate_filing_date_obj: date | None) -> list:
    ...
    search_name_input = decedent_last.strip().upper() # Using only Last Name as per request
    # If using full name:
    # if decedent_first and decedent_first.strip(): 
    #     search_name_input = f"{decedent_last.strip().upper()} {decedent_first.strip().upper()}"
    ts_print(f"[INFO] Using search name: '{search_name_input}' for Grantor/Grantee")
    for attempt in range(MAX_SEARCH_RETRIES_TARGETED + 1):
        try:
            ...
            try: # Save HTML after search and wait
                with open(OUTPUT_DIR / f"debug_targetsearch_after_click_{search_name_input.replace(' ','_')}_{attempt+1}.html", "w", encoding="utf-8") as f:
            ...
            while page_scrape_count < MAX_PAGES_PER_NAME_SEARCH:
                ...
                if page_data:
                    ...
                    # --- REMOVED BREAK for MAX_MAIN_RECORDS_TO_EXTRACT_PER_LEAD_TEST_MODE ---
                else:
                ...
                page_scrape_count+=1
                if page_scrape_count>=MAX_PAGES_PER_NAME_SEARCH: ts_print(f"[WARN] Reached MAX_PAGES_PER_NAME_SEARCH ({MAX_PAGES_PER_NAME_SEARCH}) for '{search_name_input}'. Stop."); break # Clarified log
...
def run_targeted_rp_scrape() -> pd.DataFrame: 
    ...
    all_found_property_records=[]; 
    # --- MODIFIED: STOP_AFTER_FIRST_SUCCESSFUL_LEAD set to False ---
