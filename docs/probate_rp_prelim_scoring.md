# Python Script: Advanced Record Linkage & Relevance Scoring V1.0

This document provides a detailed breakdown of a Python script designed for advanced record linkage. The script takes scraped real property and probate data, cleans it, calculates a series of feature scores, and computes a final weighted `match_score_total` to identify potential links between probate cases and property records.

## Script Overview

The primary goal of this script is to automate the process of finding property records that are likely associated with a decedent from a probate lead list. It accomplishes this by performing the following key operations:

1.  **Data Ingestion**: Loads the latest combined data file from a specified folder.
2.  **Data Cleaning**: Standardizes and cleans names to improve matching accuracy.
3.  **Feature Engineering**: Generates multiple feature scores based on:
    * Name similarity (fuzzy matching and phonetic algorithms).
    * Date proximity between the probate filing and property record filing.
    * The role of the party in the transaction (e.g., Grantor).
    * The type of legal instrument (e.g., Warranty Deed).
    * The specificity of the search that found the record.
4.  **Weighted Scoring**: Applies a configurable set of weights to the feature scores to calculate a final `match_score_total`.
5.  **Classification**: Categorizes each potential match into 'High', 'Medium', or 'Low' confidence levels.
6.  **Output Generation**: Saves a fully enriched, scored, and sorted CSV file, along with a smaller sample file for manual quality assurance (QA).
...
* **numpy**: For numerical operations, especially handling NaN values and clipping scores.
* **rapidfuzz**: For fast and flexible fuzzy string matching.
* **jellyfish**: For phonetic encoding algorithms like Soundex.
* **glob**, **os**, **pathlib**: For file system operations (finding files, creating directories).
